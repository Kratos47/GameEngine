...
void Ritter_SphereModel::privCreateVAO( const char * const modelFileName )
{
...
triList[199].v2 = 109;
	...
	Vect* pTmp = new Vect[(unsigned int)this->numVerts];
	for (int i = 0; i < this->numVerts; i++)
	{
		pTmp[i].set(pVerts[i].x, pVerts[i].y, pVerts[i].z);
		//Trace::out("%d:  %f %f %f\n", i, pTmp[i][x], pTmp[i][y], pTmp[i][z]);
	}

	// Find Bounding Volume - quick hack
	assert(this->pRefSphere);
	this->pRefSphere->RitterSphere(pTmp, this->numVerts);
...
Ritter_SphereModel::Ritter_SphereModel( const char * const modelFileName )
	: Model()
{
	this->privCreateVAO( modelFileName );
}

Ritter_SphereModel::~Ritter_SphereModel()
{
